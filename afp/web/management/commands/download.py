...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        month = args[2]
        day = args[3]
        download(administrator_code, year, month, day)

def download(administrator_code, year, month, day):

    administrator = Administrator.objects.get(code=administrator_code)
    print "getting %s-%s-%s for %s" % (year, month, day, administrator)
